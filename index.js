...
//visuels
const tamagotchi = ["ʕ •ᴥ•ʔ", "ʕ •ᴥ•ʔゝ☆", "ʅʕ•ᴥ•ʔʃ"];

const tamagotchiAngry = ["ʕ；•`ᴥ•´ʔ", "ʕ – ᴥ –ʔ"];

const tamagotchiDead = ["ʕ xᴥxʔ"];
...
function getTamagotchi() {
  ...
  } else if (state.hunger <= 45 || state.amusement <= 45) {
    //là il est vénère
    return (
      generateSpace() +
      tamagotchiAngry[Math.floor(Math.random() * tamagotchiAngry.length)]
    );
  }
  // sinon il est normal
  return (
    generateSpace() + tamagotchi[Math.floor(Math.random() * tamagotchi.length)]
  );
}
...
function getLifeBar() {
  ...
  return (
    new Array(Math.floor(plein)).fill(barCompleteChar).join("") +
    new Array(Math.floor(vide)).fill(barIncompleteChar).join("")
  );
}
...
setInterval(function () {
  ...
  if (
    (state.hunger <= 10 || state.amusement === 0) &&
    state.time % 3 === 0 &&
    state.life !== 0
  ) {
    state.life--;
